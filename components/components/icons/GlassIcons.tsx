...
// ── Additional icons to cover all remaining cases ────────────────

export const PortalIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="gold" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
      path="M12 18.5a6.5 6.5 0 100-13 6.5 6.5 0 000 13zM12 2v2M12 20v2M2 12h2M20 12h2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"
      accentDots={[{cx:38,cy:9,r:2.5,fill:"rgba(255,181,107,.9)"},{cx:9,cy:39,r:2,fill:"rgba(255,181,107,.6)"}]}/>}
  </GlassIcon>
);

export const LifecycleIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="pink" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
      path="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"
      accentDots={[{cx:38,cy:9,r:2.2,fill:"rgba(255,110,199,.85)"}]}/>}
  </GlassIcon>
);

export const SuccessCheckIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="emerald" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
      path="M22 11.08V12a10 10 0 11-5.93-9.14M22 4L12 14.01l-3-3"/>}
  </GlassIcon>
...
export const WarningIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="gold" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
      path="M10.3 3.1L2 18a2 2 0 001.7 3h16.6a2 2 0 001.7-3L13.7 3.1a2 2 0 00-3.4 0zM12 9v4M12 17h.01"/>}
  </GlassIcon>
...
export const ChallengIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="rose" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
      path="M10.3 3.1L2 18a2 2 0 001.7 3h16.6a2 2 0 001.7-3L13.7 3.1a2 2 0 00-3.4 0zM12 9v4M12 17h.01"/>}
  </GlassIcon>
...
export const SolutionCheckIcon = ({ size="md" as GlassSize, ...p }: Omit<GlassIconProps,"children"|"variant"> & {size?:GlassSize}) => (
  <GlassIcon size={size} variant="emerald" {...p}>
    {(uid,sz,colors)=><GlassSVG uid={uid} size={sz} colors={colors}
...
// Helper: maps solution index to glass icon
export function SolutionIcon({ index, size="lg" as GlassSize }: { index:number; size?:GlassSize }) {
  const Icons = [GovernanceIcon, PortfolioIcon, AIIcon, CrossBorderIcon, LifecycleIcon, PortalIcon];
  const Icon = Icons[index % Icons.length];
  return <Icon size={size}/>;
}
